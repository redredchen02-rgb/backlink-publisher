...
- **DependencyError 分支必须逐字镜像 ExternalServiceError 分支**：`_record_publish_failure → fail_count++ → outputs.append → run_id rebind`，**再** `return _ROW_CONTINUE`（`:279` 死代码转 live）。只「改控制流不补 `fail_count++`/`outputs.append`」会让该行从 stdout 与计数双双消失，`success+fail+skipped+held != len(rows)`、exit 误读 0/5 而非 4。
...
- *Error path:* adapter 抛 `DependencyError` 的行不再 `SystemExit(3)`：记 typed 失败行（`fail_count++` + `outputs.append`）；进程内消费方映射成 exit_code=3 + error_class → `tests/test_publish_engine.py`
- *Error path（深化新增）:* 中途 token-rev 漂移（`_check_token_drift` 触发）不再 `SystemExit(3)`：记 typed 中止；`terminal_exit_code==3`、`error_class` 正确 → `tests/test_publish_engine.py`
- *Edge case:* 部分成功（1 ok/1 fail）→ `success_count==1`、`fail_count==1`、`terminal_exit_code==4`、stdout JSONL **只含成功行**（镜像 `_publish_helpers.py:563`）→ `tests/test_publish_engine.py`
...
- *Edge case（深化新增·计数不变量）:* 任意 outcome 满足 `success+fail+skipped+held == len(rows)`（dependency 行不丢）→ `tests/test_publish_engine.py`
