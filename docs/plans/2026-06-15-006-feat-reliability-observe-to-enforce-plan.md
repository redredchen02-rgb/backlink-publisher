...
- [ ] **Unit 1: would-skip 数量级先验（grep，go/no-go gate）**
...
- [ ] **Unit 2: 持久化 reliability.decision 进 events.db（dispatch 接缝直写）**
...
- [ ] **Unit 3: projector 去重——`policy_skip` 不再投影成 PUBLISH_FAILED**
...
- [ ] **Unit 4: per-channel readiness reader + 计算（R1）**
...
- [ ] **Unit 5: `/ce:health` rollout 面板（R2）**
...
- [ ] **Unit 6: 退化频道主动告警（R5a，observe 期保底收益，独立于 enforce）**
