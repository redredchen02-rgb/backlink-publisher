...
- 2026-05-14: PR landing roadmap execution (plan `docs/plans/2026-05-14-004-refactor-pr-landing-roadmap-plan.md`). PR #9 (`feat/work-themed-backlinks`) merged to main (squash `e088139`) after the same canonical-vs-parallel `save_config` semantic fork as #1 emerged and was resolved by taking PR #12's Config Safety Net as the authoritative implementation. PR #10 closed as superseded; replacement PR #16 (rebased onto post-#9 main) merged (squash `afad920`). PR #17 (post-#10 `language_matches` property-test backfill, with 10000-sample structural-tautology guard verified by mutation check) merged (squash `af0e01e`). PR #1 (V1 verifier + M1 article-scope fix) **closed** after Unit 5 measurement gate fired on both signals: 4 source-file conflicts (`adapters/base.py`, `adapters/blogger_api.py`, `cli/publish_backlinks.py`, `config.py`) above the ≤3 threshold, and a clear semantic fork between PR #1's 43KB `verifier.py` vs main's 3.5KB `verify_publish.py` + `link_attr_verifier.py` (specific evidence: `adapters/base.py` `_provider_meta` typed-vs-untyped contract divergence). V1 verifier highest-value assets (SSRF defense, `_ArticleScopedCollector`, 3-layer fuzz harness) staged for extraction in `docs/plans/2026-05-14-005-feat-v1-verifier-asset-extraction-plan.md`.
