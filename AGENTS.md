...
- `docs/plans/`, `docs/brainstorms/` contain real operator domain names — don't propagate to `docs/solutions/`
	...
	- Local branches after the 2026-07-06 branch/PR merge consolidation (round 3, `docs/plans/2026-07-06-007-...-plan.md`): `main` (active — now includes PR #78 debt-registry fix, #76 PR-queue fix, #70 hidden-debt sweep, #77 attention-dashboard, #79 DataTable/pagination, #80 Windows encoding fix). 7 landed/superseded branches deleted both locally and on `origin` (`docs/convergence-006-execution`, `docs/convergence-006-u3-branch-disposal`, `fix/ruff-lint-debt`/`-v2`, `feat/u4-dual-live-route-convergence`, `fix/pr-queue-lite-error-message`/`-v2`), plus the now-merged PR branches themselves (`feat/webui-attention-dashboard`, `feat/u5-datatable-pagination-polling`, `fix/pr-queue-lite-error-message-2`, `opt/hidden-debt-hardening-sweep`, `fix/webui-windows-encoding-crash`) — every deletion tagged `archive/<branch-name>` and pushed to `origin`. **Left untouched (presumed active concurrent WIP from `docs/plans/2026-07-06-005-opt-webui-uiux-comprehensive-plan.md` and related work — do not delete without re-verifying)**: `docs/u4-test-measurement`, `feat/u6-health-dashboard-spa`, `feat/w1-refresh-defaults`, `feat/w2-settings-edit-protection`, `feat/w4-history-soft-delete`, `feat/w5-history-undo-ux`, `feat/w6-shared-form-system`, `feat/w10-cross-page-deeplink`, `feat/w12-responsive-splitscreen`, `feat/w13-mutation-error-reporting`, `chore/w14-blueprint-audit`, `fix/w15-never-run-health-projection`, `integration/w-batch-1-w1-w2-w6-w4`, `integration/w-batch-2`, `integration/2026-07-06-005-w1-w2-w4-w5-w6-w10-w12-w13-w14-w15`, `integration/w4-w5-w10-w13-reintegrate-u5`. Also untouched (pre-existing, unrelated to this round): `origin/feat/webui-console-redesign`, `origin/feat/webui-publish-workbench` (already-landed remnants from an earlier round, out of scope here).
