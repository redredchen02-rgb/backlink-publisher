...
| B6〔2026-07-06，B3/B4/B5 code review 發現，reliability/adversarial/julik-frontend-races 三方獨立收斂〕 | `frontend/src/api/prQueue.ts` 的 `updatePrStatus()` 依序等待兩個各自獨立的 15s timeout（`_csrf()` 的 token fetch，接著才是狀態更新本身的 POST）——最壞情況約 30 秒，是修復本身註解宣稱的「15s 逃生路徑」的兩倍。經比對 `webui_app/routes/spa.py`（純靜態 `send_from_directory`，非 Jinja render）與 `frontend/index.html`（完全沒有 `<meta name="csrf-token">`），確認這條「先抓 CSRF token 再送出」路徑是這個 SPA **永遠會走的正常路徑**，不是罕見的 fallback 分支 | 低～中（僅在後端明顯變慢、非徹底掛起時才會被使用者感知到接近 30 秒的等待；後端健康時兩段都很快，不影響一般使用） | 中（需要設計決策：在 `prQueue.ts` 內比照 `client.ts` 的 `csrfToken()` 快取 token，或讓兩段 fetch 共用同一個 deadline/AbortController，或明確接受目前行為） | 待處理，未排入本輪修復範圍 |
| B7〔2026-07-06，B3/B4/B5 code review 發現，correctness 提出、reliability/adversarial/julik-frontend-races 佐證〕 | `PrQueuePage.vue` 的 `markStatus()` catch block 靜默吞掉 `updatePrStatus()` 的失敗（程式內註解「Error is surfaced by load()」其實不準確——`load()` 會無條件重設 `error.value`，只有它自己的 fetch 失敗才會重新填入，不會反映 `updatePrStatus` 的失敗原因）。此為既存缺陷，但 B3 新增的 15s timeout 讓這個吞噬路徑多了一個更常見、更明確的觸發來源（逾時會直接、確定地被吞掉，而非像過去那樣無限期卡住） | 低（需要 `updatePrStatus` 剛好失敗/逾時才會踩到，但踩到後使用者完全不知道剛才的「標記為已獲得/跳過」點擊失敗了） | 中（需要 UX 決策：重用 `error.value` 顯示、per-row 提示、還是 toast，以及是否要保留目前「失敗後自動重新載入」的行為） | 待處理，未排入本輪修復範圍 |
...
〔B3/B4/B5 code review，2026-07-06，同分支追加 commit〕`ce-code-review`（10 位 reviewer：correctness/testing/maintainability/project-standards/agent-native/learnings/reliability/adversarial/kieran-typescript/julik-frontend-races）跑過一輪。reliability＋adversarial＋julik-frontend-races 三方獨立收斂到同一個真實問題（見上方 backlog B6）：`updatePrStatus()` 依序疊加兩個各自 15s 的 timeout，經比對 `spa.py`/`index.html` 確認這是永遠會走的正常路徑而非邊角 fallback，最壞情況逼近 30 秒。correctness 提出、reliability/adversarial/julik 三方佐證另一個問題（見上方 backlog B7）：`markStatus()` 的 catch block 靜默吞掉 `updatePrStatus` 失敗，且 B3 的新 timeout 讓這個吞噬路徑變得更容易被觸發。testing 提出、reliability/kieran-typescript/adversarial/julik 四方佐證四個測試覆蓋缺口，均已修復：(1) 15s timeout 的真實 `setTimeout`/`AbortController` 觸發路徑先前只用合成的立即 reject 測試，從未真正跑過真實計時器——已加 `vi.useFakeTimers()` 測試推進真實 15s 視窗；(2) `_csrf()` 的網路 fallback 分支（無 `<meta>` tag 情境）先前完全沒有測試覆蓋——已補測試涵蓋成功與失敗兩種情況；(3) B4 的既有測試只驗證了 `/app-config` 檢查點的防護，未真正驗證 B4 描述本身的核心情境（`fetchPrQueue()` 進行中的競態）——已用兩個獨立控制的 deferred promise 補測試，讓兩個 `load()` 都真正打到網路才 resolve；(4) B5 的測試只斷言全檔案字串出現次數，未釘住檢查所在的確切位置——已改為錨定到 `.then()` callback 內部的位置斷言。maintainability／kieran-typescript／julik-frontend-races 另外各提出一個 P3 討論性發現（timeout wrapper 邏輯在 `client.ts`／`prQueue.ts` 間重複、`loadGeneration` 手刻的防護模式其實 `useQuery` 就有、`load()` 沒有 `onUnmounted` 取消），均判斷為架構性/低優先序，本輪不處理。project-standards／agent-native 皆為 PASS/無阻擋發現。`npx vitest run` 全綠、`test_webui_feedback_states.py` 除既有 7 個無關的 Windows `cp950` 編碼問題外全綠。
