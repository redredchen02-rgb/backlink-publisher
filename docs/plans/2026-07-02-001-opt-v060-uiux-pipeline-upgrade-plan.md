...
**Code review（2026-07-06，12 位 reviewer 並行，base `f43dda73`）與修復：**
- **`_health_agg()` 的真實 bug（3 位 reviewer 獨立收斂：testing、correctness、reliability）**：原本只包住 `build_health()`，`project_on_read()` 本身若 raise 會直接讓整個 `_health_agg()` 拋出，導致 `_panel()` 外層把 `health`／`projection` 兩者都收斂成 `null`——而 `HealthPage.vue` 整頁用 `v-if="health && panels"` 當閘門，會把整頁悄悄清空，比 legacy Jinja 頁「暫時不可用」的誠實 fallback 還糟。修法：`project_on_read()` 也包一層 try/except，失敗時建構一個 `degraded=True` 的 `ReadProjectionResult`。新增迴歸測試。
- **`HealthPage.vue` 的 `actionBusy` 競態（2 位 reviewer 收斂：testing、correctness）**：`query.refetch()` 未 await，`finally` 立刻釋放忙碌鎖，使用者可以在畫面還沒反映新狀態前對同一個動作重複觸發。修法：`await query.refetch()`。新增迴歸測試（用可控 Promise 精確卡住 refetch 的 pending 窗口斷言忙碌狀態）。
- **`linksLoading` 共用 ref 的顯示錯亂（correctness 發現）**：展開第二個 channel 時可能清掉第一個 channel 的載入指示。修法：改成以 channel 為 key 的 reactive 物件（`channelLinks` 早已是這樣做）。新增迴歸測試。
- **`blockState` 的 `'empty'` 分支永遠不可達（kieran-typescript 發現）**：`StateBlock` 的 `empty-text` prop 因此是死代碼——不像清單頁，本儀表板即使發佈數為零也永遠有東西可顯示（每個 panel 仍會渲染，只是數值為零/空）。修法：把型別窄化成 `'loading' | 'error' | 'ready'`，拿掉不可達的 prop，並加註解說明原因。
- **文件精確度修復**：`health_dashboard.py` 模組 docstring 誤稱 `health_actions.py` 的 `_config`/`_known_platform`/`_platform_arg` 「不可 import」（kieran-python 發現此為誤——它們是模組層級函式，不像 `routes/health.py` 的 closure 真的無法 import）——修正措辭為真正原因（避免耦合到 U9 即將刪除的模組）；`_published_candidate` 的 `store` 參數型別在移植過程中從 `EventStore` 退化成 `Any`——已還原（用 `TYPE_CHECKING` guard，比照原檔慣例）；`spec.py` 的 3 個動作 description 補上「也受 app-level CSRF/Origin guard 保護」（原本只提 loopback，與同一 diff 裡 recheck-link 自己的 description 不一致，agent-native 發現）+`paused` 欄位在 reverify/circuit-reset 上被忽略的說明（api-contract 發現）；`CLAUDE.md` 關於 `/ce:health` 「Jinja-only、6 個 sub-panel」的過時描述已更新反映本 unit 的 SPA 全量對等。
- **修復後驗證**：前端 287/287（新增 2 個迴歸測試）、`vue-tsc`/`npm run lint`/build 全乾淨（**修復了一個會讓 CI 紅燈的真實 bug**——`HealthPage.vue` 有 2 個 unused-var lint 錯誤，maintainability + kieran-typescript 獨立收斂發現，`frontend.yml` 的 `npm run lint --max-warnings 0` 會擋下；測試檔本身也順手把全部 16 處 `as any` 改成正確型別標註，同一輪順便清乾淨）；後端本 unit 相關測試 407/407（新增 1 個迴歸測試）、`ruff`/`gen_openapi.py --check` 皆乾淨。
- **記入 backlog、本次不修**（皆為既有行為或超出本 unit 範圍，非本次引入的退化）：`_pipeline_summary()` 的 24h/7d/30d SQL 窗口過濾是把 TEXT 欄位（`ts_utc`）與 REAL 型別的 epoch float 比較，實測為靜默失效（三個窗口回傳同一組全量數字）——correctness 發現此為逐字複製自 `routes/health.py` 的既有 bug，兩處目前完全一致；修法需同時改兩份（`routes/health.py` 在 U9 前不應大改）留待後續統一處理；`/api/v1` 全站既有的 403 回應內容型別不一致（宣告 `application/problem+json` 但實際跑出 `application/json`，`{"error":"forbidden","status":"error"}`）——api-contract + agent-native 兩位獨立收斂確認這是**跨 5+ 個既有模組**（`bind.py`/`channel_bind.py`/`llm.py`/`settings_credentials.py`/`velog.py`）的既有落差，非本 diff 造成；pause 動作的回應是回聲客戶端輸入而非寫入後回讀（adversarial 發現，逐字沿用 `health_actions.py:75-83` 既有行為，非退化）；`pipeline_summary` 在「更多指標」通用表格渲染器裡顯示成不易讀的 JSON.stringify 區塊（correctness 發現，F2）——可考慮後續為它做一個小的專屬子表格；3 個動作的 `reason` 欄位在快樂路徑下 omit vs 空字串不一致（api-contract 發現，低優先）；`_g_cache()` 的 bare except 可能吞掉 panel 函式自己丟出的 `RuntimeError`（reliability 發現，既有共用 helper，非本檔問題）；~15 個次要 panel 的 SQL 查詢無逐一 timeout、單一 request 內循序執行的累積延遲風險（reliability 發現，屬既有查詢邏輯未變，只是首次被聚合進單一 JSON 回應——若未來有 load-test 證據顯示真的成為瓶頸再處理）。
