...
>   - ⏳ **段 2+ 剩余**：渠道段（binding 通用 5 型 + blogger/medium/velog/token-paste 卡 + cardless）、LLM 段（集成+状态+诊断按钮）、image-gen 诊断段、概览/banner/侧栏。**渠道级 save**（save-blog-ids/save-medium-token/clear-medium-token/revoke-blogger）仍 legacy——整页时补 v1 或随 U8。「Google 登入」navigate 到 legacy oauth-start（blogger oauth-start/callback 留 legacy）。各段齐→flip navItem `href`→`to`→ U8 退役 Jinja、U9 文档收敛。
> 已迁导航页（8）：发布工作台 `/`、监控 `/monitor`、历史 `/history`、草稿 `/drafts`、站点 `/sites`、排程 `/schedule`、批量 `/batch-campaign`（+ 全局 shell）。Settings 页 UI 待下一轮。
>
> **U7 两处一级护栏（随页推进维护）**：(1) `webui_app/api/v1/spec.py` 声明式 OpenAPI 契约按端点组线性增长，越 500 canary；`monolith_budget.toml` ceiling 现 **1060**（452→1054 SLOC，已含 sites/schedule/campaigns/profiles + **Settings 后端全部**：凭证写 channel-token/notion/通用 channel-credential + bind 四路由 + oauth 两路由 + llm-config + llm 诊断对 + image-gen 诊断对 + medium 浏览器登录三路由 + 全局 keywords/schedule 两路由 POST + SPA settings 页读侧 GET hydration（keywords/schedule）；受 `SEED_HEADROOM_MAX=50` 约束，后续 settings section 补 GET 会再 bump）。(2) `test_csrf_only_route_count_snapshot` 71→**98**：U5–U7 的 27 个 `/api/v1` mutating 路由（pipeline 5 + history 4 + drafts 5 + sites 2 + campaigns 1 + profiles 2 + oauth-credential 2 + llm 诊断 2 + image-gen 诊断 2 + 全局 keywords/schedule 2）计入（自 U5 潜伏 stale，因 gating lane 非该 full-suite 安全单测）。**非新漏洞**：全由 app 级 `_global_origin_guard` 运行时覆盖（`test_global_guard_covers_every_mutating_route` 绿）、且为 config/state JSON 写或操作员配置探针，非 `0600` 凭证写。**内联守卫家族**（0600 凭证写 channel-token/notion/channel-credential/bind + **medium 浏览器登录三路由**——派生进程/删 profile）保留 inline `_check_bind_origin_or_abort`→被快照排除（故未计入）。
