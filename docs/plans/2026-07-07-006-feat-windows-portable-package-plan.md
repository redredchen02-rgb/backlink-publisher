...
- **所有 CLI shim 與 WebUI 啟動腳本都設定 `PYTHONPATH=<pkg_dir>\app`（規劃時決策；Unit 2 實作時發現這行本身不會生效，見下方標註）**：直接審查原始碼發現 `dispatch_backlinks.py`、`canary/store.py`（被 `canary-targets` 透過鏈式 import 用到）等 CLI 模組會匯入 `webui_store`，而 `webui_store` 只存在於封裝的 `app\` 底下、不在 site-packages 裡。若只有 `launch-webui.bat` 設定 `PYTHONPATH` 而 CLI shim 沒有，透過 `launch-cli.bat` 呼叫這類指令會直接 `ModuleNotFoundError`。**Unit 2 實作時實測發現：embeddable 直譯器的 `python311._pth` 檔案會讓 Python 完全忽略 `PYTHONPATH` 環境變數**（用/不用 `PYTHONPATH` 執行同一支腳本，`sys.path` 逐位元組相同）。真正生效的機制改為 `ensure_app_dir_importable()`：直接把 `..\app` 這一行寫進 `python-embed\python311._pth` 檔案裡（`._pth` 限制模式下的直譯器讀這個檔案決定 `sys.path`）。生成的 `.bat` 仍保留 `set PYTHONPATH=...`（無害，也讓腳本在非 `._pth` 限制的一般 Python 上測試時仍正確），但實際運作機制以 `scripts/packaging/build_windows_package.py` 的 `ensure_app_dir_importable()` 為準，不是這一行環境變數設定。
