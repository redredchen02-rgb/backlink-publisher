...
- [ ] **Unit 1: mastodon enforce 啟用（operator-gate）**
...
- [ ] **Unit 2: 007 plist 驗收 + install runbook 完整化**
...
- [ ] **Unit 3: probe-citations 每日排程 plist**
...
- [ ] **Unit 5: debt_registry.toml mitigated 項稽查**
...
- [ ] **Unit 7: HTTP client 統一（radon 門控 + 分批收口）**
...
- [ ] **Unit 8: Decay alert（14d 掉 ≥2 條 dofollow 鏈告警）**
...
- [ ] **Unit 10: plan-gap 週排程腳本**

**Goal:** `equity-ledger | plan-gap | plan-backlinks` 每週自動執行並輸出補鏈建議，operator 確認後執行。
...
- Script 組合：`equity-ledger | plan-gap --desired N --language LANG | plan-backlinks --dry-run > <abs_path>/logs/plan-gap-latest.json.tmp && mv <abs_path>/logs/plan-gap-latest.json.tmp <abs_path>/logs/plan-gap-latest.json`（tmp→rename 確保原子寫入；`<abs_path>` 用 git root 絕對路徑，不用相對 `logs/`）
- `--dry-run` 確保不直接 publish（operator 確認後才跑真實 plan-backlinks）
...
**Verification:** `logs/plan-gap-latest.json` 生成後包含有效的 plan-backlinks seed JSONL 或空 JSONL

---

- [ ] **Unit 11: autopilot 狀態頁擴展（plan-gap 結果）**

**Goal:** sites.html 的 autopilot 區域顯示「上次 plan-gap 結果」—— 已補鏈數 / 仍缺鏈數 / 觸發時間。
...
- [ ] **Unit 12: citation share health gate（gated on Perplexity 配額確認）**
