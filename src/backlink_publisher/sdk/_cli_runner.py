"""CLI dispatch and subprocess pipeline helpers — relocated to core (plan 2026-06-22-001 U5a).

Original: webui_app/helpers/cli_runner.py (Plan 2026-05-21-007 Unit 4).
webui_app/helpers/cli_runner.py is now a re-export shim pointing here.

Path-calculation change vs the webui_app original: _REPO_ROOT uses
Path(__file__).parents[3] (sdk → backlink_publisher → src → repo-root).
The original used dirname(dirname(abspath(__file__))) which, from
webui_app/helpers/, resolved to webui_app/ — a latent bug kept inert by the
editable install.

_is_fetch_verify_disabled: inlined from webui_app/helpers/url_meta to avoid a
core→webui_app import edge; same env-var + same truthy set as _TRUTHY_BYPASS.
"""
...
_FETCH_VERIFY_TRUTHY = {"1", "true", "yes"}


def _is_fetch_verify_disabled() -> bool:
    return os.environ.get("BACKLINK_NO_FETCH_VERIFY", "").strip().lower() in _FETCH_VERIFY_TRUTHY
...
# Path fix vs the webui_app/helpers original: parents[3] from sdk/ gives the
# repo root (sdk → backlink_publisher → src → repo-root). The original
# dirname(dirname(abspath(__file__))) from webui_app/helpers/ resolved to
# webui_app/ — a latent bug kept inert by the editable install.
