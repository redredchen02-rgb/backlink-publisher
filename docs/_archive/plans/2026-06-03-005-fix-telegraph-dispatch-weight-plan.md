...
- [x] **Unit 1: registry infrastructure**
...
- [x] **Unit 2: signals + routing integration**
...
- Discount wins tiebreak: platform A (dofollow=True, weight=0.6) vs platform B (dofollow=True, weight=1.0) → B 勝（4×0.6=2.4 < 4×1.0=4.0）
- Spread strategy explicit: telegraph (dofollow=True, cover_count=0, weight=0.6) → score=(4×0.5 + 3×3.0)×0.6=(2+9)×0.6=6.6 vs ghpages (dofollow=True, cover_count=0, weight=1.0) → (2+9)×1.0=11.0 → ghpages 勝（spread formula: base×0.5 + bonus×3.0；_MAX_SPREAD_BONUS=3，故 bonus=max(0,3-0)=3；spread bonus 也被折扣）
- Telegraph binding confirmed: `collect_all()` signal for "telegraph" has `binding="bound"` AND `dispatch_weight=0.6`（Phase 1 通過 + Phase 2 折扣均生效；telegra.ph 在 `_ALWAYS_BOUND` 故永遠 bound）
...
- [x] **Unit 3: telegraph weight declaration**
